 ...
 * coroutine that started the login. There is no equivalent plumbing needed on
 * iOS -- `ASWebAuthenticationSession` hands the callback URL back directly --
 * because Android's browser-redirect-back-into-the-app flow is structurally
 * an Intent delivery, not a single API call.
 */
...
/**
 * Auth0 Authorization Code flow with PKCE, through a Custom Tab.
 *
 * Hand-rolled rather than the Auth0 Android SDK, mirroring the iOS app's own
 * choice (`Auth0Client.swift`) not to pull in the platform SDK for what is
 * three requests and a hash. A Custom Tab (not a WebView) is what keeps the
 * credentials out of a surface this app could read, same reasoning as iOS's
...
class Auth0Client(private val context: Context) {
    ...
    private fun launchCustomTab(uri: Uri): Boolean =
        try {
            ...
            // ViewModel's Application context, not an Activity one -- an
            // Application context isn't itself part of a task back stack, so
            // Android requires this flag before it will let it start an
            // activity at all.
            customTabsIntent.intent.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK)
            customTabsIntent.launchUrl(context, uri)
            true
        } catch (e: Exception) {
            android.util.Log.e("IsekaiFFI", "launchCustomTab failed", e)
            false
        }
