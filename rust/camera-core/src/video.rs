...
impl AttestedPeer {
    ...
    pub fn from_connection(connection: &PeerConnection) -> Option<Self> {
        let peer_endpoint = connection
            .peer_endpoint
            .clone()
            .or_else(|| connection.target_endpoint.clone())?;
        Some(Self {
            attestation: connection.video_attestation.clone()?,
            peer_endpoint,
            video_host: connection.video_host.clone()?,
        })
    }
...
fn install_pin(conn: &Connection, pin: AttestedPeer) {
